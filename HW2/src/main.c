...
//ADC Channels
#define ADC1_EXAMPLE_CHAN0          ADC1_CHANNEL_2 // GPIO34
...
void app_main(void)
{
    ...
    adc_chars = calloc(1, sizeof(esp_adc_cal_characteristics_t)); //Allocate memory for ADC calibration
    esp_adc_cal_characterize(unit, atten, width, DEFAULT_VREF, adc_chars); //Calibrate ADC
    while (1) {
       // clock_gettime(CLOCK_REALTIME, &start); //Start time
        ...
        uint32_t voltage = esp_adc_cal_raw_to_voltage(adc_raw[0][0], adc_chars);

        //ESP_LOGI(TAG_CH[0][0], "raw data: %d ", adc_raw[0][0]); //Print raw data
        ESP_LOGI(TAG_CH[0][0], "voltage: %d ", voltage); //Print raw data
